		...
		for a in range(0,stot):
			for b in range(0,stot):
				...
				if newstate in di:
					...
					v1 = v1 + increment1*((stot)**2 + ((stot)**2-1)*peso*(float)(di[newstate]))/((stot)**2 + ((stot)**2-1)*peso*(float)(di[state]))
					v2 = v2 + increment2*((stot)**2 + ((stot)**2-1)*peso*(float)(di[newstate]))/((stot)**2 + ((stot)**2-1)*peso*(float)(di[state]))
				else:
					# Se non c'e mai stata questa transizione di stati
					v1 = v1 + increment1*((stot)**2)/((stot)**2 + ((stot)**2-1)*peso*(float)(di[state]))
					v2 = v2 + increment2*((stot)**2)/((stot)**2 + ((stot)**2-1)*peso*(float)(di[state]))
	else:
...
for i in range(0,len(portfolios)):
	file.write(str(i) + " : " + str(portfolios[i]) + "\n")
